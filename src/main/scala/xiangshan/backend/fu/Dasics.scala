...
class DasicsBackendIO(implicit p: Parameters) extends XSBundle with DasicsConst {
...
class DasicsBackend(implicit p: Parameters) extends XSModule with DasicsMethod with HasCSRConst {
  val io: DasicsBackendIO = IO(new DasicsBackendIO())
