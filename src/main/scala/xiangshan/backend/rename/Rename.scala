...
object RenameZeroRewrite {
  // ADR 0004 extends the original untrusted-only rewrite into the untrusted
  // context window, where trusted handler code is saving/restoring that context.
  def rewriteContext(dasicsUntrusted: Bool, untrustedContextWindow: Bool): Bool =
    dasicsUntrusted || untrustedContextWindow

  def shouldRewrite(dasicsEn: Bool, dasicsUntrusted: Bool, untrustedContextWindow: Bool, initBit: Bool, srcType: UInt): Bool =
    dasicsEn && rewriteContext(dasicsUntrusted, untrustedContextWindow) && !initBit && SrcType.isRegOrFp(srcType)
