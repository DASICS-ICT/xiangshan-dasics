...
object lastBranchState {
  def initial = "b00".U   //last fetch packet has no jump/taken branch (fall-through packet) 
  def flushed = "b01".U   //last fetch packet has a jump/taken branch, but it is flushed (require checking) 
  def valid   = "b10".U   //last fetch packet has a jump/taken branch, and it is valid
}
...
class Ftq(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper
  ...
  val lastBranchStateReg  = RegInit(lastBranchState.initial)
  ...
  def lastBranchNotNull = lastBranchStateReg =/= lastBranchState.initial
  ...
  lastBranchActive      := lastBranchStateReg === lastBranchState.valid || 
                           (lastBranchStateReg === lastBranchState.flushed && 
  ...
  when (io.toIfu.req.fire) {
    ...
    when(lastBranchShouldFlush){
      ...
      lastBranchStateReg := lastBranchState.initial
    }.elsewhen (lastBranchStateReg === lastBranchState.valid) {  
